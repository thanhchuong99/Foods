...
interface NumberSeven {
  keyframes?: Keyframes;
}

const NumberSeven = ({ keyframes }: NumberSeven) => {
  return (
    <Box
      id="number-4"
      sx={() => ({
        ...
        left: 'calc((536 - var(--border-width)) / 1122 * 100%)',
        top: 'calc((271 - var(--border-width)) / 848 * 100%)',
        animation: generateAnimation(keyframes),
        boxShadow: '0 20px 20px 5px rgba(92, 110, 141, 0.24)',
        borderRadius: '50%',
      })}
    >
      <Image fill alt="carrot" src="/assets/home-hero/number-7.png" />
    </Box>
  );
};
