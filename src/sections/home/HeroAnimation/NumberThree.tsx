...
interface NumberThreeProps {
  keyframes?: Keyframes;
}

const NumberThree = ({ keyframes }: NumberThreeProps) => {
  return (
    <Box
      id="number-3"
      sx={() => ({
        ...
        left: 'calc((879.24 - var(--border-width)) / 1122 * 100%)',
        top: 'calc((683.24 - var(--border-width)) / 848 * 100%)',
        animation: generateAnimation(keyframes),
        transform: 'rotate(30deg)',
      })}
    >
      <Image fill alt="carrot" src="/assets/home-hero/number-3.png" />
    </Box>
  );
};
